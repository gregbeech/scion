...
describe Scion::Headers::AcceptCharset do
  ...
  context '::parse' do
    it 'can parse a basic charset range' do
      header = Scion::Headers::AcceptCharset.parse('unicode-1-1;q=0.8')
  ...
  context '#merge' do
    it 'can merge two headers with the right precedence' do
      h1 = Scion::Headers::AcceptCharset.parse('unicode-1-1;q=0.8')
      h2 = Scion::Headers::AcceptCharset.parse('iso-8859-5')
      header = h1.merge(h2)
      expect(header.charset_ranges.size).to eq(2)
      expect(header.charset_ranges[0].to_s).to eq('iso-8859-5')
      expect(header.charset_ranges[1].to_s).to eq('unicode-1-1; q=0.8')
